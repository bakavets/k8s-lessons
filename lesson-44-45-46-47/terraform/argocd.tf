...
resource "kubectl_manifest" "initial_bootstrap" {
  yaml_body = templatefile("${path.module}/templates/argocd-initial-bootstrap.yaml", {
    ...
    bootstrapApp_externalSecretsConfigs_serviceAccountName = local.external_secrets.service_account_name,
    bootstrapApp_externalSecretsConfigs_namespace          = local.external_secrets.namespace,
    bootstrapApp_externalSecretsConfigs_eksRoleArn         = module.iam_assumable_role_external_secrets.iam_role_arn
